...
namespace Mission.Services.Services
{
    public class MissionService(IMissionRepository missionRepository, IMissionSkillRepository missionSkillRepository) : IMissionService
    {
        ...
        {
            var m = await _missionRepository.GetByIdAsync(model.Id);
            if (m == null) return false;

            m.CityId = model.CityId;
            m.CountryId = model.CountryId;
            m.EndDate = model.EndDate;
            m.MissionDescription = model.MissionDescription;
            m.MissionImages = model.MissionImages;
            m.MissionSkillId = model.MissionSkillId;
            m.MissionThemeId = model.MissionThemeId;
            m.MissionTitle = model.MissionTitle;
            m.StartDate = model.StartDate;
            m.TotalSheets = model.TotalSeats;
            m.ModifiedDate = DateTime.UtcNow;

            await _missionRepository.SaveAsync(); // ✅ This line is required

            return true;
        }

        public string MissionDelete(int id)
        {
